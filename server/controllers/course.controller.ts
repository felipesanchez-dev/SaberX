      ...
      if (isCacheExist) {
        try {
          ...
          return res.status(200).json({
            success: true,
            message:
              "Curso cargado desde la memoria del caché (Se evitó una consulta a la base de datos innecesaria)",
            cached: true,
            course,
          });
        } catch (error) {
...
export const getCourseByUser = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!courseExists) {
        return next(
          new ErrorHandler("No eres elegible para acceder a este curso", 403)
        );
      }
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!mongoose.Types.ObjectId.isValid(contentId)) {
        return next(
          new ErrorHandler("El contenido no tiene un ID válido", 400)
        ); // Error 400 (Bad Request)
      }

      const courseContent = (course?.courseData as unknown as any[])?.find(
        (item: any) => item._id.equals(contentId)
      );

      if (!courseContent) {
        return next(
          new ErrorHandler("El curso no contiene este contenido", 404)
        );
      }
...
export const addAnswer = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!question) {
        return next(
          new ErrorHandler("El curso no contiene esta pregunta", 404)
        );
      }
      ...
      if (req.user?.id === question.user._id) {
        ...
        try {
          await sendEmail({
            ...
            subject:
              "Se ha añadido respuesta a tu pregunta en uno de nuestros cursos",
        ...
        res.status(200).json({
          message:
            "Respuesta añadida exitosamente, se ha enviado un mensaje de notificación",
          success: true,
          course,
        });
      }
...
export const addReview = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!courseExists) {
        return next(
          new ErrorHandler("Usted no tiene acceso válido a este curso", 404)
        );
      }

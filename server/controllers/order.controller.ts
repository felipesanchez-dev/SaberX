  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (courseExistInUser) {
        return next(
          new ErrorHandler("El usuario ya tiene este curso en su cuenta", 400)
        );
      }
      ...
      try {
        if (user) {
          await sendEmail({
            email: user.email,
            subject:
              "🎉 ¡Felicidades! Tus cursos han sido agregados con éxito 🚀",
        ...
        console.error(
          "Error al enviar correo de confirmación de orden:",
          error
        );
